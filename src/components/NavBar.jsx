  ...
  return (
    <header className="bg-black p-3 text-white flex items-center justify-around ">
      <nav className="flex items-center justify-center gap-8">
        <Link to={pagePaths.todo}>Todo</Link>
        <Link to={pagePaths.movie}>Movies</Link>
      </nav>
      <div>
        <strong>TOTAL:</strong>
        <span className="text-2xl ml-2">{totalTasks?.length}</span>
      </div>
      <div>
        <strong>COMPLETED:</strong>
        <span className="text-2xl ml-2">{completedTask?.length}</span>
      </div>
      <div>
        <strong>PENDING:</strong>
        <span className="text-2xl ml-2">{pendingTask?.length}</span>
      </div>
    </header>
  );
};
